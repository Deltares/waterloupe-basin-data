...
select fi.sub_parameter, fi.parameter, fi.users, fi.scenario, fi.solution, pe.period_id, pe.period_name, sd.date, a.area_id, a.name as area, a.km2 as area_km2, a.geometry, sd.value
...
select sub_parameter, parameter, scenario, solution, area_id, area, array_agg(round(sum_value::numeric,2) order by date) as data, array_agg(distinct date order by date) as date_agg
from (
    select sub_parameter, parameter, scenario, solution, area_id, area, sum(value) as sum_value, avg(value) as avg_value, date
